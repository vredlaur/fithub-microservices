...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ApiError> handleFeign(FeignException exception) {
        log.error("Communication error with gym-service", exception);
        return ResponseEntity.status(HttpStatus.BAD_GATEWAY)
            .body(ApiError.of(502, "Nu s-a putut comunica momentan cu serviciul de clase fitness."));
    }

    @ExceptionHandler(Exception.class)
    public ResponseEntity<ApiError> handleUnexpected(Exception exception) {
        log.error("Unexpected booking-service error", exception);
